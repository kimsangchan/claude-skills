...
GATE 검토 서브에이전트 1명 약 20만). A0에서 강도를 판정해 `00-seed.md`에 `강도: spike|lite|full (사유)`로 적는다.
...
| 실측·상한 토큰 | 실측 20만 (2026-09-07 S1: 검색 5·스킬 호출 11·도구 호출 48·서브에이전트 0·문서 74KB). 상한 25만 | 실측 약 70만 (2026-09-07 S4: 메인 37.5만 + 조사 sonnet 11.6만 + 검토 5만 + GATE 16.7만, 3.5시간). 상한 80만(서브에이전트 포함). 넘으면 멈추고 사용자에게 보고 |

lite의 바닥은 스킬 본문 읽기와 도구 호출 횟수다 (문서 분량이 아니다). 더 줄이려면 스킬 호출을 단계당 1개로, 파일 쓰기를 묶어서.

**spike** — 플랫폼·범위·핵심 루프 중 하나라도 미확정이면 신호 표와 무관하게 spike 먼저 (2026-09 한삼국 실전: 법·하드웨어 신호로 full을 돌렸지만
실제로는 탐색 단계였고 엔진·범위가 하루 만에 뒤집힘). spike = A0(해석 목록) · A1(검색 ≤5, 참조 제품 규칙 깊이·데이터 공급 표본) · A2(질문 최대 2: 사용 맥락·최상위 해석) ·
...
6. **비용 기록.** 런이 끝나면 decision-log 말미에 `강도 · 검색 횟수 · 서브에이전트 수 · 소요 시간`(알면 토큰도)을 적는다.
   다음 강도 판정의 근거가 된다.
7. **단계마다 `NEXT.md` 갱신.** 각 단계를 마칠 때 `autopilot/<slug>/NEXT.md`에 현재 상태 1줄 · 다음 할 일 3줄 · 재개 명령 1줄을 덮어쓴다.
   세션이 끊기면 이 파일이 재개 포인터다(실행 절차 5). 08이 없어도 다른 세션이 이어받을 수 있어야 한다.
8. 최종 보고는 (0) **해석 목록**(A0에서 내가 정한 것 — "아니면 지금 고치세요"), (a) 판정, (b) 핵심 결정 5줄 요약,
